...
| Route | Description |
|---|---|
| `/` | **Public marketing landing page** — hero (word-by-word rising headline + Join Wishlist form + a brand Saturn whose **ring** spins as the user scrolls, planet body fixed) with ambient orbit dots and a scroll cue, count-up stats strip, four feature showcase sections (adaptive planner / interactive sessions + pacing clock / automated error log / Question Bank manager) threaded by a **central purple timeline spine** (scroll-driven fill + glowing comet head + per-feature nodes that light at viewport center), each with an illustrative in-code product mock, "how it works" strip, closing CTA, footer. Slow, staggered scroll-reveal animations throughout and a header scroll-progress bar; all driven by one rAF loop (paused when the tab is hidden) with full `prefers-reduced-motion` / no-JS fallbacks. Logged-in users are redirected to `/home` by `app/page.tsx`; logged-out users see the landing page. Renders `components/marketing/landing-page.tsx`. The wishlist email form is the only live backend interaction. |
