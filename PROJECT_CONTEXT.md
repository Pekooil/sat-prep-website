...
| Route | Description |
|---|---|
| `/` | **Public marketing landing page** — hero (word-by-word rising headline + "Get started free" + "Sign in" CTA buttons + a brand Saturn whose **ring** spins as the user scrolls, planet body fixed) with ambient orbit dots and a scroll cue, count-up stats strip (real user count from `users` table + question inventory count), four feature showcase sections (adaptive planner / interactive sessions + pacing clock / automated error log / Question Bank manager) threaded by a **central purple timeline spine** (scroll-driven fill + glowing comet head + per-feature nodes that light at viewport center), each with an illustrative in-code product mock, "how it works" strip, closing CTA with direct signup button, footer. Slow, staggered scroll-reveal animations throughout and a header scroll-progress bar; all driven by one rAF loop (paused when the tab is hidden) with full `prefers-reduced-motion` / no-JS fallbacks. Logged-in users are redirected to `/home` by `app/page.tsx`; logged-out users see the landing page. Renders `components/marketing/landing-page.tsx`. No waitlist form — the app is publicly available. |
...
| `/settings` | Notification preferences (email + in-app reminders, timezone, reminder types); **danger-zone self-service account deletion** (`DeleteAccount` → `actions/account.ts`) |
| `/privacy`, `/terms` | Public legal pages (`app/(legal)/`); copy + contact details sourced from `lib/legal/config.ts` |
...
- **Marketing landing page** (`/`) — public, logged-out landing page that replaces the old `/`→`/login` redirect. Built with the "Quiet Monochrome" design tokens and `components/ui` primitives: sticky top bar (logo + Features anchor + Join Wishlist + discreet Sign in) with a scroll-progress bar, hero with the primary wishlist email form and a brand Saturn whose **ring spins as the user scrolls while the planet body stays fixed** (dashed-ellipse `stroke-dashoffset` inherited by the ring segments + an orbiting moon; scroll term + ambient drift, idle float, ambient orbit dots, scroll cue), a 4-stat count-up strip (8 domains / 2 subjects / 0 repeats / 100% free), **four feature showcase sections** — adaptive planner, interactive sessions + SAT pacing clock, automated error log, Question Bank coverage manager — threaded by a **central purple timeline spine** (scroll-driven fill + glowing comet head + per-feature nodes that light at viewport center; centered between the alternating columns on desktop, a left rail on mobile), each pairing copy + bullets with an illustrative in-code product mock in a browser-chrome frame (copyright-safe: skeleton bars stand in for question content, only public CB domain/skill labels and A/B/C/D letters appear, each captioned "no SAT content is ever shown or stored"), a copyright-safe "plan → practice with CB Question Bank → log & adapt" strip, a closing wishlist CTA, and a footer. IntersectionObserver-driven directional scroll-reveal animations (`.lp-reveal` primitives in `globals.css`) with full `prefers-reduced-motion` + no-JS fallbacks (content defaults to visible). Fully responsive, dark (default) + light. Wires up nothing in the app — the **only** live action is the wishlist email capture (`actions/waitlist.ts` → `waitlist_signups`). `app/page.tsx` keeps the server-side `getUser()` check and still redirects authenticated users to `/home`. The old `/`→`/home` rule in `next.config.ts` was removed so `/` reaches the page
- **Legal compliance (US-only, Session 22)** — 13+ age gate with under-18 parental acknowledgement enforced server-side (`lib/legal/config.ts validateAgeConsent`) on both signup paths; `users.birth_year`/`terms_accepted_at`/`parental_ack`; self-service account deletion in Settings; tightened Privacy (CCPA/SOPIPA/cookies) + Terms; Privacy/Terms links + SAT-trademark disclaimer across landing/login/signup; waitlist consent microcopy; CAN-SPAM email footer; non-blocking cookie notice. Central contact/entity placeholders in `lib/legal/config.ts`. See `LEGAL_COMPLIANCE.md`.
- **Authentication** — email/password sign up + sign in + sign out via Supabase Auth; Google OAuth sign-in/sign-up (`signInWithOAuth` → `/auth/callback` → `/auth/google-consent` for new users); `proxy.ts` protects all dashboard routes
