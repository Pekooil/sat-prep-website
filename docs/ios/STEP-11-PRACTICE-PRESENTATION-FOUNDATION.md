...
**Status:** Native mock-backed presentation complete; live practice transport, lifecycle recovery, and physical-device validation remain blocked
...
- persist the active question, timer, and selection across process termination;
...
- Thirty native unit-test cases passed, including practice start, response requirement, elapsed timing, one-request submission guarding, idempotency-key creation, server-owned feedback/summary transitions, and normalized error states.
- Three native UI tests passed on an iPhone 17 Pro simulator, including Home → Start → question → feedback → summary → Home.
...
When the practice OpenAPI milestone is `READY`, generate or implement the versioned Swift transport models, add the live repository, map the public answer-free question and server feedback payloads into these presentation types, and add response fixtures. Then connect `PracticeRecoveryStore` to the view model, test background/foreground and termination recovery, exercise network-loss replay, and verify the full path on a signed physical iPhone before checking I6 complete.
